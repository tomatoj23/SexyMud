...
- **两侧都住**：内容条目 → 注册表内建倒排索引 `byTag(dimension, key) → id[]`，覆盖**所有带 tags 的实体**——**四个集合的条目 ＋ 出口**（**定案 2026-09-02，答 #15 的边界提问**：连出口一起索引。依据：出口 id 全局唯一、与条目 id 是**同一个 id 空间**（出口 id 即分发键，spec/02 §4），混排不会撞——**且「不会撞」由注册表强制**（#15 follow-up：四个集合与出口的 id 同属**一个唯一性空间**，跨集合重名在加载期大声失败、报错点名两侧类型；此前它只是内容约定，`byTag` 会把重名的两个实体静默并成一行）；M3-T1 已为出口开了 schema 口子〔出口即命令〕，只写不查等于留一个死字段。引擎不认识集合，只认带 tags 的实体）；运行时实体 → `EntityState` 加 `tags` 槽（与 `flags` 并列），`subjectOf` 据此回答 `hasTag`——`hasTag` 由**桩**（恒 `false`）变真实现，`has_tag` 谓词第一次有真实数据可读。
...
- **落地（M3-T6，#19）**：
  - **维度表随包，由装载器一起交出**：迷你包（`tests/fixtures/mini-pack/config/dimensions.json`）声明**三个维度名完全不同的维度**（`section`／`clearance`／`hazard`），与武侠包那张表零重合；`packRegistry` 用同一个装载函数读 `config/`，**换目录即换维度表**。机械验证（同一份迷你包内容 × 三张表）：配自己的表 → 过；不配表 → 跳过校验；配武侠包的表 → 大声失败（未知维度）。反向也钉住：往迷你包条目写一条武侠维度（`moveTag`）→ 失败，不配表则照样加载（封闭是**可选的**，有表才有错）。
  - ⚠️ `schemas/config.dimensions.schema.json` 的 `required` 名单点名的是**武侠包自己的维度**——它是那张表的契约，由 `content:check` 对 `content/` 执行。第二套包的表按同一 schema 的**形状**那一半校验（维度名 lowerCamelCase、键列表非空去重），取值封闭归注册表。**是否要为每个包各配一份 config schema，是尚未决的开放问题**（迷你包是测试夹具，不在 `content:check` 的扫描范围内，故本票未动那张 schema）。
  - **继承来的标签进索引，用真实内容钉住**：基类 `room-orb-000` 一个文件写 `clearance: ["crew"]`，`byTag("clearance","crew")` 答 **5 个房间**（展平先于建索引）；`byTag("clearance","eva")` 答 `["exit-orb-001-outboard", "room-orb-004"]`——**出口 id 与条目 id 混在一张结果里**，真实内容行使了 #15 的那条定案。
  - **`has_tag` 门禁第一次有真实内容**：气闸门 `exit-orb-001-outboard` 挂 `{ "has_tag": ["clearance", "eva"] }`（新二元组形态），拒绝文案 `err_traverse` 留在迷你包 JSON；测试直接往状态树写／删标签断言放行与再拒绝（照 #17 的先例），并断言同词 flag 不开门。
...
- **加载期展平，在注册表内**（ADR-0030 §1–§2）：顺序 = **`id` 去重（跨集合同一空间）→ 展平 → 引用完整性校验 → 建 `byTag` 索引**（末两步见 §5.1 落地条：索引在 #15 之后才存在，且必须晚于展平）。顺序不是审美——继承来的 `exits`、放置清单、`monsterId` **也必须被引用校验**，否则「把字段放进原型」就是绕过校验的后门；继承来的 `tags` 同理必须进索引，否则「把标签放进原型」就是绕过索引的后门。主机侧做展平则 M2-T6 的「同一装配路径」从**结构保证**退化成**纪律**。
...
- **落地（M3-T6，#19）**：
  - **真实继承链**：迷你包四个舱室（`room-orb-001`–`004`）继承同一个「舱室基类」`room-orb-000`（`prototypeKey` = 自身 id，原型就塞在 `rooms/` 集合里，不开 `prototypes/`）。基类**不带出口**（构造性，见上一条），带的是放置清单与 `tags`：**生活舱／主控室不声明 `objects` ⇒ 拿到基类的壁挂检修机**；**对接舱声明自己的巡检机器人**（`objects` **整体替换**，不是并集——只有 `tags`／`attrs` 互补合并）；**舷外作业平台声明 `objects: []`**（显式覆盖为空，与「没声明」不是一回事）。展平后四个子房间都没有 `prototypeParent`，也都没有 `prototypeKey`（ ⇒ 不可被继承）。引擎源码本票**零改动**。
  - ⚠️ 房间集合的 `name`／`description`／`enterText`／`exits` 四项是 schema **必填** ⇒ 子类必然自己声明一遍，**「继承描述／进入文本」在真实内容里不可观测**；可观测的继承载荷是 `tags`／`flags`／`objects`／`err_*`／`preconditions`／`zoneId` 这几项（房间基类自身的那份文案是为满足 schema 必填而写的）。
  - **环检测的离线那一半已落**（ADR-0030 §5 的双保险自此两边齐了）：`scripts/check-content.mjs` 按集合遍历 `prototypeParent` 图，成环即失败并**打印环**（自环／二环／长环；**菱形不是环**——同一祖先被两条路径到达只算一次）。只查环：父是否存在、父是否声明 `prototypeKey` 是展平器加载期的事（离线重复实现一遍等于造两份真相，必然漂移）。沿既有先例**不新增单元测试**（四道检查都没有）。
  - ⚠️ **范围限定**：`content:check` 只扫 `content/`（已发布内容），迷你包是**测试夹具**，不在它的扫描范围内——本票的验收写法（「往迷你包插环 → content:check 报环」）按现状做不到。手工验证的做法是临时把迷你包 5 个房间文件复制进 `content/rooms/` 跑一遍门禁（无环 → `OK prototype graph`；给基类加一条指向孙子的 `prototypeParent` → 报 `room-orb-000 → room-orb-004 → room-orb-000` 并退出码 1；改成自环同样报出），验完删除。**`content/` 至今零原型，这道离线门禁今天仍无内容可守**；要让迷你包也进 `content:check`，先决条件是解决 `config.dimensions.schema.json` 的 `required` 名单（它点名武侠包的维度，第二套包的表过不去）——留作开放问题。
- **落地（M3-T3，#16）**：`packages/core/src/content/prototype.ts` 的 `flattenCollection(条目数组, 集合名)` = 展平器，由 `createContentRegistry` 在 **id 去重之后、引用完整性之前**按集合各调一次，四步顺序 `id 去重（跨集合同一空间）→ 展平 → 引用完整性校验 → 建 byTag 索引` 已在注册表落地（注册表文件头写明「顺序是契约，不是排布」）。**合并律**：`tags`／`attrs` **互补合并**（`tags` 按维度取键的并集；`attrs` 按键取并集、同键由高优先级赢），其余键**整体替换**；多亲左→右、自身最后（自身 > 最右父 > … > 最左父）；合并产生的数组**字典序升序 + 去重**。展平结果**剥掉 `prototypeParent`**、**只保留自身声明的 `prototypeKey`**（父的不继承）→ 未声明者展平后即不可被继承。加载期大声失败四种：`prototypeKey` ≠ 本条目 id（对每个条目都查，与遍历顺序无关）、父 id 不在本集合（**同集合内**继承）、父未声明 `prototypeKey`、原型成环（自环／二环／长环抓；**菱形不是环**——同一祖先被两条路径到达只算一次）。**没声明 `prototypeParent` 的条目原样穿过展平**（同一个对象引用，零拷贝、零重排）——今天所有内容都属于这一类，故零原型的包在加载期不付任何代价；`prototypeParent: []` 算**已声明**，照样走展平并剥掉（留着它会让运行时以为还有继承没做）。`attrs` 仍**不进 schema**，只由合成测试行使（ADR-0030 §7）。接缝：`tests/content-registry.test.ts`（既有缝，合成条目驱动 19 例）。⚠️ 一处未随本票落地（另一处已由 #19 补上：环检测的离线那一半已落，见上面 T6 落地条）——**出口不可继承是构造性的，不是待办**：`schemas/rooms.schema.json` 把 `exits` 定为**必填** + `exits` 走整体替换 + 出口 id 全局唯一 ⇒ schema 合法内容永远不会继承出口；即便绕过 schema 省略 `exits`，两个房间共用同一个出口对象也会在出口 id 去重处报 `duplicate exit id`（而各房间的边目标本来就不同，共用本身也无意义）。**基类原型只能带描述／进入文本／标签／放置清单这类字段，出口由各房间自己声明**（#19 的真实继承链按此设计：基类只带放置清单与标签）。出口**自身的** `prototype*` 仍无消费者（展平按集合做）
- **落地（M3-T1，#14）**：`prototypeKey`／`prototypeParent` 与 `tags`／`flags` 一起进 `schemas/common.schema.json`（见 §5.1 落地条）：四个字段是**同一份实体层契约**的两半（条目带，出口也带——出口即命令），14 个条目集合统一引用，一律**可选**。schema 表达不了的三件事（`prototypeKey` 是否等于本条目 id、父是否声明过原型、是否成环）留给注册表展平（**#16 已落**）与 `content:check` 环检测（**#19 已落**）。
...
- **`WorldRuntime`**（`packages/core/src/world/runtime.ts`）：ContentRegistry ＋ 状态树 ＋ hook 载体实例三合一，**外加高水位 `clock`**（M4-T3：它就是「驱动世界的那一侧」，见 `spec/04` §2.2；`addEntity` 用它给 `lastSeenTick` 种子）；`occupantsOf` 按 id 升序（确定性）；`subjectOf` 从树状态构造条件主题（**flags／tags／location 真实回答**——tags = 自身槽 ∪ 内容条目（M3-T5），未落地槽位答「无」）。两条注册路径：`addEntity`（**创建**——注册＋种子状态进树，跑 creation 两层的是 `createObject`）与 `attachEntity`（**恢复**——树已有状态，只重挂实例，位置须仍能解析，顺序无关；M2-T5）。
...
- [x] 标签**不带值**（形状 `{<维度>: [键…]}`，取值由维度表封闭、**由注册表**硬校验，schema 只管形状）；归属用字段不用目录分层——M3-T2 已落：形状归 schema（M3-T1），取值封闭归注册表（维度表可选传入，传了才校验）；**M3-T6 已落「维度表随包」**：迷你包声明三个与武侠包零重合的维度，换目录即换表，同一内容配别包的表即大声失败
- [x] `(维度, 键)` 倒排索引可跨内容批量查询（注册表 `byTag`，覆盖所有带 tags 的**实体**：条目 ＋ **出口**，id 升序混排）——M3-T2 已落；**M3-T6 用真实内容钉了一次**：基类一个文件写的键查出 5 个房间（展平先于建索引），`byTag("clearance","eva")` 一条结果里同时有出口 id 与房间 id
...
- [x] 原型合并：`attrs`/`tags` 互补，其余整体替换；合并后**字典序升序 + 去重**——M3-T3 已落：`content/prototype.ts` 的 `flattenCollection`（`tags` 按维度取键并集、`attrs` 按键取并集同键高优先级赢，其余键整体替换；多亲左→右、自身最后）；**M3-T6 用真实内容钉了一次**：迷你包四个舱室继承一个「舱室基类」，不声明 `objects` 的两个拿到基类的放置清单，声明了的整体替换（含显式 `[]`），`tags` 则互补合并
- [x] 展平在**加载期、注册表内**，顺序为 `id 去重 → 展平 → 引用完整性校验`——M3-T3 已落：四步顺序（末步索引见 §5.1）钉在 `content/registry.ts` 文件头与展平调用处
- [x] `content:check` 有**原型环检测**；注册表展平时也防环（双保险）——**M3-T6 已落**：离线那一半在 `scripts/check-content.mjs`（按集合遍历 `prototypeParent` 图，成环即失败并打印环，菱形不误报，沿先例无单测、手工验证过）；运行时那一半在展平器（自环／二环／长环，M3-T3）。⚠️ 离线这道今天**无内容可守**：`content/` 零原型，迷你包是测试夹具不在 `content:check` 的扫描范围内
