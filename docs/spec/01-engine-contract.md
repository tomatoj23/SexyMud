...
### 2.3 `tick` 由宿主产生、由命令携带（M4-T1，ADR-0031）

- 宿主把墙钟翻译成 tick，**只在构造 `Command` 时给它**；引擎不再被注入时钟。
- tick 是**非负安全整数**，不是毫秒、不是时间戳；非法值由引擎大声失败（接线错误，不是玩家输入）。
- 引擎取所有**非 `invalid`** 的命令 tick 的**高水位**作为唯一的「现在」；倒退的命令照常执行，只是不把世界往回拨。
- 与 `seq` 分工：**`seq` 定投递顺序、`tick` 定世界时间**，二者独立、不互相校验（`spec/04` §2.5）。
- 高水位**必须进存档**（v2，ADR-0033）：不存则恢复后时间倒退，冷却永远不到期。

依据：ADR-0031、ADR-0033；详见 `04-state-and-time.md` §2.2
...
packages/core/          引擎（可独立发布的库，零题材词）
  src/
    ...
    time/               tick 高水位（clock.ts）、游戏内时间求值（calendar.ts／tuning.ts，M4-T2）
                        —— 调度六原语待 #22／#23
    effects/            效果执行
