...
- **装配**（`commandSpecFromEntry`）：`id`→`key`、`argForm`→解析段、`preconditions`→access 门禁。**询问的 accessType 由宿主传入**（词汇表是内容侧约定：命令 `use`，见 content.md），引擎只携带不拥有；条目带门禁而未给 accessType = 装配 bug，大声抛错。执行体（func）：穿行由**引擎出厂适配器** `traversalSpec` 提供（ADR-0028，**M2-T1 已落**：traverse 门禁 → 目标房 enter 门禁 → `moveTo`，门禁外置）；看/说适配器随后（M2-T2/T3）；其余命令仍由宿主注入（效果管线落地前）。**执行段拒绝通道**（M2-T1 新增）：`func` 可返回 `CommandRejection {kind: "rejected", reason}`——执行期合法拒绝（目标房 enter 门禁、移动 hook 否决）映射为 `rejected` 结果，拒绝事件由拒绝方自己发（执行者拥有全部语境：commandKey／errKey／roomId），管线不补发通用文案；`at_post_cmd` 在拒绝时不运行。`CommandContext` 新增 `predicates`（谓词注册表随 deps 注入，执行段自查门禁与 access 段同源）。
