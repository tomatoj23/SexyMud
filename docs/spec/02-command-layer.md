...
- **装配**（`commandSpecFromEntry`）：`id`→`key`、`argForm`→解析段、`preconditions`→access 门禁。**询问的 accessType 由宿主传入**（词汇表是内容侧约定：命令 `use`，见 content.md），引擎只携带不拥有；条目带门禁而未给 accessType = 装配 bug，大声抛错。执行体（func）：走/看/说/穿行由**引擎出厂适配器**提供（ADR-0028，M2 起）；其余命令仍由宿主注入（效果管线落地前）。
...
- **出口 = 命令 + 边**：引擎类型 `ExitEntry extends CommandEntry`（`packages/core/src/world/entry.ts`）——出口携带全套命令字段（verbs／argForm（恒 `none`）／cmdset／priority／mergetype／preconditions／`err_*`），另加 `direction`（边的方向标签，同房内唯一）与 `targetRoomId`。出口经**同一条装配路径**进入分发：`commandSetSources(exits)` 分组、`commandSpecFromEntry(exit, { accessType: "traverse", func })` 装配（穿行行为由**引擎出厂适配器**提供，ADR-0028：traverse 门禁 → 目标房 enter 门禁 → `moveTo("traverse")`，门禁外置于 `moveTo`）。
