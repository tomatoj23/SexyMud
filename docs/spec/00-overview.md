...
**下一步**：~~M2 最小可玩世界运行时（走/看/说，六张 tracer 票，形态见 ADR-0028）~~（**六票全关**）→ **M3 标签与原型（spec/03 §5.1／§6.1）—— 设计已定案（ADR-0029／ADR-0030），已拆六票（#14–#19，规格快照 #13）** → M4 时间与调度（spec/04 §2–§4，战斗前夜）。每完成一个子系统，跑该文件末尾的自检清单。
