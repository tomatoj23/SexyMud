...
| 放置期集合的 schema（14 个，口径见 `HANDBOOK`） | ⚠️ 放置期设计，需随本规格重估（`monster.schema.json` 已随 `mon-lq-001` 进入编译，重估仍未做）。**限定语**：`schemas/` 现共 **19 个** = 2 个被引用库（`condition`／`common`）+ 3 个新落集合（`commands`／`rooms`／`npcs`）+ `config` 三类 + **11 个**放置期集合；「14」= 19 − 2 库 − 3 新落（口径不同、数字都对，别去统一它们） |
