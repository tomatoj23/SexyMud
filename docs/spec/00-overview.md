...
| 快照 v1：状态树序列化 + 迁移链往返（M2-T5） | ✅ **已落**（`state/snapshot.ts`：`serializeWorld`／`restoreWorld`——载荷即状态树（非平行结构），只加版本戳／`derived` 切分／规范序（同世界＝同字节）；`state/derived.ts`：`derived` 一张表同时驱动快照类型与序列化排除，加载后逐实体 `recompute`（表今日为空，首个消费者＝修饰符系统）；`WorldRuntime.attachEntity`：**恢复＝重放树＋重挂实例，不走 `createObject`**（creation 两层不跑，否则代码默认值覆盖存档），挂载顺序无关、内容漂移大声失败；当时 `SAVE_VERSION` 保持 1、迁移链机制就绪但为空（v2 由 #24 落地）；NPC **构造性不入档**（静态在场＝内容真相）；`tests/snapshot.test.ts`） |
...
| **时间／调度 · T5：存档 v2 + `Rng.getState()` + 首条真实迁移（M4-T5，#24）** | ✅ **已落**（`save/migrations.ts`：`SAVE_VERSION` 1 → **2**，`migrations[1]` 是这条链的**首条真实迁移**（此前机制就绪但为空，ADR-0003 要求不造假迁移），总规则是「**只补缺失的、绝不覆盖已有的**」—— ADR-0033 §3 写 `lastSeenTick = nowTick` 时那个槽还不存在于 v1，而 #22 之后写的 v1 存档带着它，覆成人 0 等于白送一整个世界的离线补算。`Rng` 端口加**强制**的 `getState(): number`，`createSeededRng(state)` 一身二用（新建＝恢复），不另开 `restoreRng`。**O9 定死**：顶层槽走 `serializeWorld(world, meta)`（**`meta` 必填**）／`restoreWorld(snapshot) → { state, meta }`，`WorldMeta = { nowTick, rngState, due }` —— 与 §2.2「高水位住在驱动侧」一致，不让 `WorldState` 背一口它不拥有的钟；被否的 (a)（标量并进 `WorldState`）与 (c)（读写都经 runtime）理由见 `spec/04` §1.5 约定 3。`SaveDataV2` 装 `nowTick`／`rngState`／`due`（`due` 是 #23 的交接，ADR-0033 §2 只列了三样）＋ 每实体 `lastSeenTick`；恢复一律「缺即空」，且**不做 v2 → v3 连迁**（v3 存档直接大声失败）。`tests/save-v2.test.ts` 13 例 + `tests/migrations.test.ts` 补 7 例；全量规模见 `HANDBOOK`「当前事实」） |
| **时间／调度（＝ M4，T6–T7）** | ⏳ **设计已定案、部分未实现**（19 条，见 `spec/04` §2–§4 与 ADR-0031–0034；T1–T5 已关，余 #25–#26） |
