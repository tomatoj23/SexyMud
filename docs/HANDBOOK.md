...
| `docs/adr/0001`–`0034` | 34 个架构决策（0016–0030 = MUD 转向与引擎定位；**0029 标签模型、0030 原型继承** = M3；**0031 tick 真相、0032 日历内容化、0033 存档 v2、0034 两层推进** = M4） | ✅ 权威 |
...
| `schemas/` | JSON Schema **20 个**：`config` 拆 4 类：dimensions / display-tiers / settings / calendar（日历，M4-T2 新增）；两个**被引用库**：`condition`（条件表达式）、`common`（条目通用字段 tags／flags／prototypeKey／prototypeParent，14 个条目集合统一 `$ref`）；`content/` 现有 **16 个 JSON**（4 config + 4 commands + 4 rooms + 3 npcs + 1 monster），均通过校验 | 🚧 **12 个**无内容映射（= 2 个被引用库 + 10 个尚无内容的放置期集合）；放置期集合 **11 个**（那 10 个 + `monster.schema.json`，它已进编译但重估未做） |
...
| 内容管线 | `content/` 共 **14 个条目集合**（原 11 + `commands/` `rooms/` `npcs/`）+ `config/`（结构性配置）+ `lore/`（纯 Markdown）；每条目一 JSON 文件；id 一经发布不可变更。详见 `docs/agents/content.md` | `content.md` |
...
| **交互模型** | 命令层内容化（`commands/` 的动词/别名/前置/拒绝文案全是数据，**引擎零动词**）；世界 = 房间（`rooms/`）+ 人物（`npcs/`）；**双时钟**（心跳 tick ／ 离线结算）—— ⚠️ 「不共用代码路径」已被 **ADR-0032** 覆盖为**同一个推进函数的两个跨度**；保留的隔离是**语义**上的：离线只补资源与基础熟练度，不自动战斗／不推层／不产掉落 | ADR-0016 ／ **ADR-0032** |
...
| **状态持久化** | 持久（进存档）／非持久（重启即失，**游戏逻辑不得读取**）。⚠️ Evennia 的 `db.*`／`ndb.*` 叫法**不采用**——`spec/04` §1.3 只保留其精神、**丢弃其 API**（本作是 typed 状态对象 + `derived` 标记）。**未显式写入的字段不落盘**，改默认值时既有存档自动跟随 | ADR-0022 |
...
| **游戏内时间** | 时辰／刻／季节全是 `nowTick` 的**纯函数**，渲染时推导、**绝不存储**；**历法本身是内容**（`calendar.json`），换包即换历法。**〔M4-T2 已落〕** | ADR-0025／**ADR-0032** |
...
| **日历与时间换算** | **全内容化**：日历 = `content/config/calendar.json`（**一组独立的环**），调参留 `settings.time`；缺失时**引擎侧大声失败、无默认公历**。**〔M4-T2 已落〕** ⇒ 细节**只在** `spec/04` §3 | ADR-0032 |
| **存档 v2** | `Rng` 加强制 `getState()`；v2 增 `nowTick`／`rngState`／`due`（到期项，#23 交接）／实体 `lastSeenTick`；顶层槽经 `WorldMeta` 进出（`serializeWorld(world, meta)`／`restoreWorld → { state, meta }`，**O9**）；恢复时一律「缺即空」，**不做 v2→v3 连迁**。⇒ 细节**只在** `spec/04` §1.5 | ADR-0033 |
...
**明确不做**：知识/生活技能、换派偷师、招架/格挡（留 v2——出处 CONTEXT.md「闪避」条：命中判定两级，非 ADR-0008）、`itemLevel`（ADR-0008）、**任务只做进度翻译型**（完成条件只映射现有系统的可观测量，不做对话树、不做任务道具独立类型）。⚠️ ~~独立 NPC 集合~~这一项**已由 ADR-0016 翻案**（立 `content/npcs/`，现有 3 条内容），留此仅为历史留档
...
- **包**：`@sexymud/*` —— `packages/core`（端口与契约 / 命令层 / 世界内容契约 / **实体运行时 + 移动 hook（M2-T1）** / **看行为：`return_appearance` + `at_look` + look 出厂适配器（M2-T2）** / **说行为：`at_msg_receive` + `broadcastMessage` 投递缝 + say 出厂适配器（M2-T3）** / **接缝补全：get/give/drop 转移配对 + creation 两层 + 动态 cmdset `assembleSources`（M2-T4）** / **状态树种子 + 快照 v1（`serializeWorld`／`restoreWorld` + `derived` 契约 + `attachEntity` 重挂，M2-T5）** / **第二内容包验收：非武侠迷你包经同一装配路径跑通走/看/说（M2-T6）** / **标签与原型 T1：四字段 schema 与类型落地（`content/entry.ts` 的 `EntryCommon`，M3-T1）** / **标签与原型 T2：内容侧标签倒排索引 `byTag` + 维度表硬校验（`content/registry.ts`，M3-T2）** / **标签与原型 T3：加载期原型展平 `flattenCollection`（`content/prototype.ts`，M3-T3）** / **标签与原型 T5：运行时标签——状态树 `tags` 槽 + `hasTag(维度, 键)` 并集 + `has_tag` 二元组谓词 + tags 进存档（M3-T5）** / **标签与原型 T6：迷你包验收——异种维度表 + 真实继承链 + `content:check` 离线环检测（M3-T6，`tests/mini-content-pack.test.ts` 23 例 + `scripts/check-content.mjs`；引擎零改动）** / **时间 T1：`Command` 携带 tick + 高水位 `TickClock` + `Clock` 翻转（`src/clock.ts`，M4-T1）** / **时间 T2：日历内容化与求值（`content/config/calendar.json` + `schemas/config.calendar.schema.json` + `src/content/config.ts` ＋ `src/time/calendar.ts` 的 `createGameTime` ＋ `src/time/tuning.ts` 的 `createTimeTuning`，M4-T2）** / **时间 T3：推进 `settleTo` + 两层推进 + 离线结算接缝（`src/time/settle.ts` ＋ 状态树 `lastSeenTick` 槽 ＋ `WorldRuntime.clock` ＋ `parseCommand` 驱动侧预检，M4-T3）** / **时间 T4：到期桶 `due.ts` + 冷却 `cooldown.ts`（状态树 `cooldowns` 槽）+ 纯 stage 求值 `stage.ts`（M4-T4）** / **时间 T5：存档 v2（`SAVE_VERSION` 1 → 2）+ `Rng.getState()` + 首条真实迁移 + O9 签名（`serializeWorld(world, meta)`／`restoreWorld → { state, meta }`，M4-T5）** / 存档迁移链）+ `apps/web`（React + Vite 壳）+ `apps/editor`（占位）；**28 个测试文件 / 594 用例全绿**
- **Schema 20 个**：`config` 拆 4 类（dimensions / display-tiers / settings / calendar）+ **2 个被引用库**（`condition` 条件表达式、`common` 条目通用字段〔M3-T1，14 个集合 `$ref` 引用〕）+ `commands`／`rooms`／`npcs`（M1-T5/T6 新落）+ 11 个集合（放置期设计）。**口径限定**：20 总数／15 待重估（= 4 config + 11 放置期集合）/ 12 无内容映射（= 2 库 + 10 个无内容集合；`monster` 有内容故不计入）——三个数字划分标准不同，都对，别去统一它们
- **`content/config/` 4 个文件**：`dimensions.json`（10 个维度）、`display-tiers.json`（造诣 50 档，**已逐项比对 xkx100 §5.1 原表**）、`settings.json`（空壳——数字随消费它的系统落地）、`calendar.json`（M4-T2：一组独立的环，日内十二时辰 `day` ＋ 年内四季 `year`，环周期 = Σ 段 tick 数）
...
| ~~🚧 世界模型实体运行时~~（**已全关**，本行留档；新待办不写这里） | **M2 六张票已全关**：T1 实体+移动 tracer（#7，已落）、T2 看（#8，已落：`return_appearance` 纯组装 + `at_look` 可见性 + `lookSpec` 出厂适配器）、T3 说（#9，已落：`at_msg_receive` 可否决 + `fromObj` 可空的 `broadcastMessage` 投递缝 + `at_pre_say`／`at_post_say` 配对 + `saySpec` 出厂适配器）、T4 接缝补全（#10，已落：`getObject`/`giveObject`/`dropObject` 三配对包 `moveTo` 外 + `createObject` creation 两层〔顺序即契约，JSON 赢代码默认值〕+ `assembleSources` 动态 cmdset〔逐分发重组零缓存〕，全合成驱动测试）、T5 快照 v1（#11，已落：`serializeWorld`/`restoreWorld` 载荷即状态树〔规范序＝同世界同字节〕+ `derived` 一张表驱动类型与排除、加载后重算 + `attachEntity` 恢复＝重放树不跑 creation + NPC 构造性不入档 + 未来版本与七类损坏载荷大声失败（逐条测试行使）；**当时** `SAVE_VERSION` 保持 1、迁移链机制就绪但为空 —— v2 与首条真实迁移由 **#24** 落地）、T6 非武侠迷你包验收（#12，已落：迷你包经同一装配路径跑通走/看/说 + 门禁拒绝 + 零武侠词 + 两包互不渗漏；验收标准 2 首次机械化）；形态定案 ADR-0028；余下＝标签（spec/03 §5）与原型（spec/03 §6）——**设计已定案（ADR-0029／ADR-0030）、已拆 M3 六票（#14–#19，规格快照 #13）；T1 schema 与类型落地（#14）已关**（`schemas/common.schema.json` 唯一定义 + 14 个集合 `$ref` + 引擎 `EntryCommon` + `content.md` 同步；`tests/tags-prototype-schema.test.ts` 77 例逐集合守卫形状）；**T2 内容侧标签（#15）已关**（`content/registry.ts` 内建 `byTag` 倒排索引，覆盖四个集合的条目 **＋ 出口**、id 升序混排；维度表可选传入、传了硬校验没传跳过；`flags` 不进索引；`tests/content-registry.test.ts` 合成驱动 11 例）；**T3 原型展平（#16）已关**（`content/prototype.ts` 的 `flattenCollection`，注册表在 id 去重后、引用完整性前按集合调用；`tags`/`attrs` 互补合并〔字典序 + 去重〕、其余整体替换、多亲左→右自身最后；展平剥 `prototypeParent`、只留自身 `prototypeKey`；大声失败四种——`prototypeKey` ≠ id、跨集合引用、父未声明 `prototypeKey`、成环〔菱形不误报〕；无 `prototypeParent` 的条目原样穿过（`prototypeParent: []` 是**已声明**、照样剥掉）；`tests/content-registry.test.ts` 合成驱动 19 例）；**T4 编排（#18）已关**（接线与顺序已在 #16 落地，superseded，残留价值转成 #16 follow-up 的 2 条例用）；**T5 运行时标签（#17）已关**（`EntityState.tags: TagMap`〔`addEntity` 种子带空槽〕+ `subjectOf` 的 `hasTag(维度, 键)` = **自身 ∪ 注册表 `tagsOf(id)`**〔新增 `id → TagMap` 直查，与 `byTag` 同一趟构建、未知 id 答 `{}`〕+ `has_tag` 谓词改 `[维度, 键]` 二元组〔三处同步：`conditions.ts`／`condition.schema.json` 新增 `tagPredicateNode` 并把 `has_tag` 摘出单字符串谓词组／spec/02 §5.3，旧单字符串写法 schema 拒绝；真实内容零 `has_tag`〕+ tags 进存档〔`serializeWorld` 规范序、恢复可选缺即空、`flags` 保持必填，口径写进 spec/04 §1.4〕；`tests/tags-runtime.test.ts` 17 例 + 既有回归网 ~25 处改形状）；**T6 迷你包验收（#19）已关**（迷你包补 `config/dimensions.json`：`section`／`clearance`／`hazard` 三维度与武侠包零重合，装载器读 `config/` 并把维度表交给注册表 ⇒ 换目录即换表；四个舱室继承「舱室基类」`room-orb-000`（基类不带出口，只带放置清单与 `tags`）；基类一处写的键经 `byTag` 查出 5 个房间，一条结果里出口 id 与房间 id 混排；气闸门挂 `has_tag: ["clearance","eva"]`，文案留在 JSON；`scripts/check-content.mjs` 补离线环检测）。⚠️ 遗留一处：**出口不可继承是构造性的**（`rooms.schema.json` 把 `exits` 定为必填 + 整体替换 + 出口 id 全局唯一 ⇒ 两个房间共用同一出口对象必然报 `duplicate exit id`，基类原型只能带描述／文本／标签／放置清单，出口各房间自己声明） |
...
3. **M3 已拆票（六票全关，spec/03 §8 自检清单 19 条全勾）**（设计定案见 `spec/03` §5.1／§6.1 + **ADR-0029**／**ADR-0030**；规格快照＝**#13**）：~~T1 schema 与类型落地（**#14**）~~（**已关，3c4a470**）→ ~~T2 内容侧标签（**#15**）~~（**已关，2776306**）／ ~~T3 原型展平（**#16**）~~（**已关，e83a064**；follow-up `3cf1630`）→ ~~T4 编排（**#18**）~~（**已关**：接线与顺序已在 #16 落地，残留价值转成 #16 follow-up 的 2 条例用；详见 #18 关票评论）→ ~~T5 运行时标签（**#17**）~~（**已关，37cab68**）→ ~~T6 迷你包验收（**#19**）~~（**已关，7dccf0f**）。注：票号不连续（#17＝T5、#18＝T4），因为并行建票；票正文的阻塞边写的是 **M3-Tn 简称**不是数字，故不受影响
4. **M4 ＝ 时间与调度（spec/04 §2–§4，战斗前夜）—— 下一个里程碑，设计已定案、已拆七票，T1／T2／T3 已关**：2026-09-08 的 `grill-with-docs` 访谈 19 条全部记入 **`spec/04` §2–§4 正文 + §0 术语表**，定案见 **ADR-0031（tick 真相与 Clock 翻转）／0032（日历全内容化 + 双时钟收敛为同一推进函数的两个跨度）／0033（存档 v2 载荷与 `Rng.getState()`）／0034（两层推进：世界层 vs 实体层）**。⚠️ ADR-0031 覆盖了 `spec/01` 端口表 `Clock` 的「宿主实现」一列；ADR-0032 覆盖了 ADR-0016 §4 的「双时钟不共用代码路径」（被保留的那一半是「离线只补资源与基础熟练度，不自动战斗／不推层／不产掉落」）。**已拆七票（#20–#26，全部带 `ready-for-agent` 与 GitHub 原生阻塞边）**：~~**#20** M4-T1 `Command` 携带 tick + `Clock` 翻转（无阻塞）~~（**已关，43c028b**）→ ~~**#21** M4-T2 日历内容化与求值（无阻塞）~~（**已关，2fd9ba7**）→ ~~**#22** M4-T3 推进 `settleTo` + 两层推进（阻塞：#20 已关）~~（**已关，d70bc89**）→ ~~**#23** M4-T4 到期桶 + 冷却 + stage 求值（阻塞：#22 已关）~~（**已关，f908dd1**）→ ~~**#24** M4-T5 存档 v2 + `Rng.getState()` + 首条真实迁移（阻塞：#20／#22／#23 已关）~~（**已关，7a11431**）→ **#25** M4-T6 迷你包异种日历验收（阻塞：#22；#21 已关）→ **#26** M4-T7 宿主 Clock/Authority（阻塞：#20，`apps/web` 已获开工授权）。**#21（M4-T2）已关，2fd9ba7**：日历内容化与求值 —— `content/config/calendar.json` ＋ `schemas/config.calendar.schema.json`（第 20 个 schema，一组独立的环、环周期 = Σ 段 tick）；注册表收 `settings?`／`calendar?` 并原样读出、加载期校验 schema 管不了的那一层；`src/time/calendar.ts` 的 `createGameTime`（半开区间 + 一次取模，**缺日历首次使用才大声失败**）与 `src/time/tuning.ts` 的 `createTimeTuning`（定死 O3）；迷你包自带异种日历 ⇒ 换目录即换历法。**#22（M4-T3）已关**：推进 `settleTo` + 两层推进 + 离线结算接缝 —— `src/time/settle.ts` 的 `createSettler({ state, clock, world?, entity? })`→`settleTo({ toTick, seq, actorId? })`（**在线心跳／离线补算／到期桶是同一函数的三种跨度**；世界层从驱动侧高水位补到 `toTick`、实体层只推进 `actorId` 且取他自己的 `lastSeenTick`，跑完由引擎写回、宿主不碰；`actorId` 缺省即宿主心跳，只跑世界层）；状态树新增 `lastSeenTick` 槽（v1 存档里可选，照 `tags` 先例）、`WorldRuntime` 持有 `clock` 且 `addEntity` 用**当前 tick** 种子；`parseCommand` 驱动侧预检 ⇒ `invalid` **连推进函数都不调用**（驱动侧四步：预检 → 推进 → 分发 → `observeDispatch`，`spec/04` §4.1）；**O1 定死**：`GameEvent` 必带 `tick`（命令事件写它看到的现在、结算事件写 `dueTick` 不写补跑时刻，规则进 `spec/01` §5.0）；`tests/settle.test.ts` 22 例。**#23（M4-T4）已关**：到期桶 + 冷却 + 纯 stage 求值 —— `src/time/due.ts` 的 `createDueBucket({ fire })`（载荷 `{ dueTick, payload }`、**payload opaque 进存档**、按 `dueTick` 升序稳定触发；触发口径 `dueTick <= toTick`，与冷却同一条 `now >= due`；事件 tick 由引擎盖 `dueTick`，处理器的 `DueEvent.tick` 是 `never` ⇒ 无从写补跑时刻）＋ `src/time/cooldown.ts` 的 `cooldownReady`／`cooldownRemaining`（`nowTick >= dueTick` 的** tick 比较**，不是回调）＋ 状态树 `cooldowns` 槽（`addEntity` 种子 `{}`、v1 存档可选缺即空、键排序规范序、第九类畸形载荷大声失败）＋ `src/time/stage.ts` 的 `stageAt`（线性、末端夹紧，与环相反）；到期桶接的是 #22 那个世界层注册位（`createSettler({ world: bucket.settle })`，`settleTo` 一行未改）。**O4**（到期桶全局扁平、不提供按房间/区域索引，键列表由测试钉死）与 **O8**（宿主处理器经 `CommandDeps.due` 注入，命令侧只拿 `DueScheduler`，缺失时布雷大声失败）一并定死回写 `spec/04` §6；**大跨度不加 `maxCatchUpTicks`**，由「100 万 tick ⇒ 世界层 1 次／处理器 3 次 + 耗时上界」＋「`src/` 与 `content/config/` 无此旋钮」两条测试钉死。`tests/due.test.ts` 17 例 ＋ `tests/cooldown.test.ts` 14 例 ＋ `tests/stage.test.ts` 11 例。⚠️ 留一笔给 #24：**到期桶的 v2 槽位**不在这两处名单里，#24 定夺 `serializeWorld`／`restoreWorld` 签名时一并接上（本票只造好 `snapshot()`／`restore()` 边界）—— **已在 #24 接上**（`SaveDataV2.due`）。**#24（M4-T5）已关**：存档 v2 + `Rng.getState()` + 首条真实迁移 —— `SAVE_VERSION` 1 → 2，`save/migrations.ts` 的 `migrations[1]` 是这条链的**首条真实迁移**（此前机制就绪但为空，ADR-0003 要求不造假迁移），总规则「**只补缺失的、绝不覆盖已有的**」（ADR-0033 §3 写 `lastSeenTick = nowTick` 时那个槽还不存在于 v1，而 #22 之后写的 v1 存档**带着**它；覆成 0 等于白送一整个世界的离线补算）；`Rng` 端口加**强制** `getState()`、`createSeededRng(state)` 一身二用（新建＝恢复）；**O9 定死**为 `serializeWorld(world, meta)`（`meta` 必填，写入侧 `nowTick` 过 `assertTick`）／`restoreWorld(snapshot) → { state, meta }`，`WorldMeta = { nowTick, rngState, due }`，结论回写 `spec/04` §1.5 约定 3；`SaveDataV2` 装 `nowTick`／`rngState`／`due` ＋ 每实体 `lastSeenTick`，恢复一律「缺即空」（代价：v2 存档丢 `nowTick` **不报错**，游戏从第 0 tick 继续 ⇒ 冷却永远不到期），**不做 v2 → v3 连迁**（v3 直接大声失败）。`tests/save-v2.test.ts` 13 例 + `tests/migrations.test.ts` 补 7 例。**当前前沿：#25（#22 已关，阻塞解除）与 #26**，两条都可开工。开放问题 **O1–O10** 已逐条分配到票内（O8 → #23、O9 → #24、O5 → #26）；**O1 已由 #22 定案、O3 已由 #21 定案、O7 已由 #21 结清、O4 与 O8 已由 #23 定案、O9 已由 #24 定案**；O6（时间谓词）明确不进 M4；O10 的宿主那一半归 #26。**O10**（`runCommand` 算出的 `nowTick` 要不要回传驱动侧；`spec/04` §6，#20 复查新提，已定 (b)：驱动侧一律经 `observeDispatch`）→ **#22 已落驱动侧那一半（`WorldRuntime.clock` + harness 四步）／#26 落宿主 `Authority` 那一半**
