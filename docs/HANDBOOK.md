...
4. **M4 ＝ 时间与调度（spec/04 §2–§4，战斗前夜）—— 下一个里程碑，设计已定案、已拆七票，T1 已关**：2026-09-08 的 `grill-with-docs` 访谈 19 条全部记入 **`spec/04` §2–§4 正文 + §0 术语表**，定案见 **ADR-0031（tick 真相与 Clock 翻转）／0032（日历全内容化 + 双时钟收敛为同一推进函数的两个跨度）／0033（存档 v2 载荷与 `Rng.getState()`）／0034（两层推进：世界层 vs 实体层）**。⚠️ ADR-0031 覆盖了 `spec/01` 端口表 `Clock` 的「宿主实现」一列；ADR-0032 覆盖了 ADR-0016 §4 的「双时钟不共用代码路径」（被保留的那一半是「离线只补资源与基础熟练度，不自动战斗／不推层／不产掉落」）。**已拆七票（#20–#26，全部带 `ready-for-agent` 与 GitHub 原生阻塞边）**：~~**#20** M4-T1 `Command` 携带 tick + `Clock` 翻转（无阻塞）~~（**已关，43c028b**）→ **#21** M4-T2 日历内容化与求值（无阻塞，可与 #20 并行）→ **#22** M4-T3 推进 `settleTo` + 两层推进（阻塞：#20）→ **#23** M4-T4 到期桶 + 冷却 + stage 求值（阻塞：#22）→ **#24** M4-T5 存档 v2 + `Rng.getState()` + 首条真实迁移（阻塞：#20／#22／#23）→ **#25** M4-T6 迷你包异种日历验收（阻塞：#21／#22）→ **#26** M4-T7 宿主 Clock/Authority（阻塞：#20，`apps/web` 已获开工授权）。**当前前沿：#21（无阻塞）与 #22（#20 已关，阻塞解除）**。开放问题 O1–O9 已逐条分配到票内（O8 → #23、O9 → #24、O5 → #26）；O6（时间谓词）明确不进 M4
