...
| 🚧 世界模型实体运行时 | **M2 已拆六张票**（实体+移动 tracer／看／说／接缝补全／快照 v1／非武侠迷你包验收）；形态定案 ADR-0028；标签（spec/03 §5）与原型（§6）排 M3 |
...
2. **M2 已拆票**（ADR-0028 定形态）：实体运行时+移动全链路（tracer）→ 看 → 说 → 接缝补全 → 快照 v1 → 非武侠迷你包验收（验收标准 2 首次机械化）；阻塞边：T1 → {T2,T3,T4,T5}，{T2,T3} → T6
