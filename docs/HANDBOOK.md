...
3. **M3 已拆票**（设计定案见 `spec/03` §5.1／§6.1 + **ADR-0029**／**ADR-0030**；规格快照＝**#13**）：~~T1 schema 与类型落地（**#14**）~~（**已关，3c4a470**）→ ~~T2 内容侧标签（**#15**）~~（**已关，2776306**）／ ~~T3 原型展平（**#16**）~~（**已关，e83a064**；follow-up `3cf1630`）→ ~~T4 编排（**#18**）~~（**已关**：接线与顺序已在 #16 落地，残留价值转成 #16 follow-up 的 2 条例用；详见 #18 关票评论）→ ~~T5 运行时标签（**#17**）~~（**已关，37cab68**）→ T6 迷你包验收（**#19**）；**阻塞边：T1 → {T2,T3}，T2 → {T4,T5}，{T4,T5} → T6**（T4 已关，#19 现只待 #17）。注：票号不连续（#17＝T5、#18＝T4），因为并行建票；票正文的阻塞边写的是 **M3-Tn 简称**不是数字，故不受影响
