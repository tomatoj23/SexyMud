...
| **原型继承** | **加载期展平、注册表内、同集合**，顺序 `id 去重 → 展平 → 引用完整性校验`；`prototypeKey` = 条目 id，展平剥掉 `prototypeParent`；环检测双保险。**〔M3-T3 已落〕** `content/prototype.ts` 的 `flattenCollection`：`tags`/`attrs` 互补合并（字典序 + 去重）、其余整体替换、多亲左→右自身最后；无 `prototypeParent` 的条目原样穿过；环检测（菱形不误报）已落，**`content:check` 那道离线环检测尚无票认领** | ADR-0030 |
